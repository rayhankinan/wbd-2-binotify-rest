...
const parseFrameHeader = (header: Buffer) => {
    const b1 = header[1];
    const b2 = header[2];
    ...
    const bitRate =
        bitRates[bitRateKey as keyof typeof bitRates][bitRateIndex] || 0;

    const sampleRateIdx = (b2 & 0x0c) >> 2;
    const sampleRate =
        sampleRates[version as keyof typeof sampleRates][sampleRateIdx] || 0;

    const sample =
        samples[simpleVersion as keyof Samples][layer as keyof Sample];
