...
function frameSize (samples: number, layer: number, bitRate: number, sampleRate: number, paddingBit: number) {
  if (layer === 1) {
...
function parseFrameHeader (header: Buffer) {
  ...
  const bitRate = bitRates[bitRateKey as keyof typeof bitRates][bitRateIndex] || 0

  const sampleRateIdx = (b2 & 0x0c) >> 2
  const sampleRate = sampleRates[version as keyof typeof sampleRates][sampleRateIdx] || 0

  const sample = samples[simpleVersion as keyof Samples][layer as keyof Sample]
  ...
  return {
    ...
    frameSize: frameSize(sample, parseInt(layer), bitRate, sampleRate, paddingBit),
    samples: sample
  }
}
